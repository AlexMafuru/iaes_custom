...
def get_columns(filters):
    ...
    if mode == "Daily Detail":
        base += [
            ...
            {"fieldname": "day_name",        "label": _("Day"),           "fieldtype": "Data",  "width": 60},
            ...
            {"fieldname": "day_status",      "label": _("Status"),        "fieldtype": "Data",  "width": 110},
        ]
    else:
        base += [
...
def execute(filters=None):
    ...
    daily_map    = _build_daily_map(checkins)
    if mode == "Daily Detail":
        data = _build_detail_rows(employees, daily_map, working_days)
    else:
        data = _build_summary_rows(employees, daily_map, working_days)
...
def _get_holiday_set(employees, from_date, to_date):
    ...
    for emp in employees:
        hl = emp.get("holiday_list") or frappe.db.get_single_value("HR Settings", "default_holiday_list")
        if not hl or hl in seen:
            continue
        seen.add(hl)
        rows = frappe.get_all("Holiday",
            filters={"parent": hl, "holiday_date": ["between", [from_date, to_date]]},
            fields=["holiday_date"])
        for r in rows:
...
def _build_summary_rows(employees, daily_map, working_days):
    ...
    for emp in employees:
        emp_id = emp["name"]
        present = late_count = early_count = missing_count = 0
        ...
        avg_wh  = flt(total_wh / present_for_avg, 2) if present_for_avg else 0.0
