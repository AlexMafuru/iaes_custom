...
def get_data(filters):
    ...
    rows = frappe.db.sql("""
        SELECT
            ...
            COALESCE(p.estimated_costing, 0)      AS project_value,
        ...
        LEFT JOIN (
            SELECT project, SUM(net_total) AS so_value, COUNT(name) AS so_count
            FROM `tabSales Order`
        ...
        LEFT JOIN (
            SELECT project,
                SUM(net_total)                        AS sinv_value,
                SUM(grand_total - outstanding_amount) AS sinv_paid,
        ...
        LEFT JOIN (
            SELECT project,
                SUM(net_total)                        AS pinv_value,
                SUM(grand_total - outstanding_amount) AS pinv_paid,
        ...
        LEFT JOIN (
            ...
            WHERE se.docstatus = 1
              ...
              AND se.stock_entry_type IN (
                  'Material Issue','Material Transfer for Manufacture',
                  'Manufacture','Send to Subcontractor')
            GROUP BY se.project
